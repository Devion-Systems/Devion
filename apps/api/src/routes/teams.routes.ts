...
routes.get("/:orgSlug/teams", async (c) => {
  ...
  const items = await db
    .select()
    .from(team)
    .where(eq(team.organizationId, scope.org.id))
      ...
      items.map(async (item) => ({
        ...
        memberCount: (
          await db
            ...
            .from(teamMember)
            .where(eq(teamMember.teamId, item.id))
        ).length,
        projectCount: (
          await db.select({ id: projects.id }).from(projects).where(eq(projects.teamId, item.id))
        ).length,
      })),
    ),
  );
...
routes.get("/:orgSlug/team-members", async (c) => {
  ...
  return c.json(
    await db
      ...
      .where(eq(member.organizationId, scope.org.id))
...
routes.post("/:orgSlug/teams", async (c) => {
  ...
  await db
    .insert(team)
    .values({ id, name: parsed.data.name, organizationId: scope.org.id, createdAt: new Date() });
  await db
    .insert(teamMember)
...
routes.get("/:orgSlug/teams/:teamId", async (c) => {
  ...
  const [members, assignedProjects] = await Promise.all([
    ...
    db
      ...
      .from(projects)
      .where(eq(projects.teamId, item.id))
...
routes.post("/:orgSlug/teams/:teamId/members", async (c) => {
  ...
  const orgMember = await db.query.member.findFirst({
    where: and(eq(member.organizationId, scope.org.id), eq(member.userId, parsed.data.userId)),
  });
...
routes.delete("/:orgSlug/teams/:teamId/members/:userId", async (c) => {
  ...
  await db
    ...
    .where(
      and(
        eq(teamMember.teamId, c.req.param("teamId")),
        eq(teamMember.userId, c.req.param("userId")),
      ),
    );
