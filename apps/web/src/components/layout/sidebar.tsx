...
import { ChevronDown, ChevronLeft, Settings, LogOut } from 'lucide-react'
import { cn } from '@/lib/utils'
import { getOrgNavGroups, adminNavGroups, type NavGroup } from '@/config/nav'
    ...
    >
      ...
      <div className="flex h-14 items-center justify-between border-b border-zinc-800 px-3">
        {!collapsed && variant === 'org' ? (
          ...
          !collapsed && <span className="px-2 text-sm font-semibold text-zinc-100">Devion Admin</span>
        )}
        <button
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        isActive ? 'bg-zinc-900 text-zinc-100' : 'text-zinc-400 hover:bg-zinc-900 hover:text-zinc-200'
      )}
    >
      <Icon className="h-4 w-4 shrink-0" />
      {!collapsed && <span className="truncate">{item.label}</span>}
      {isActive && !collapsed && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-emerald-500" />}
    </Link>
  )
}
...
function useOrgNavGroupsFiltered(): NavGroup[] {
  const { org, membership } = useOrgContext()
  const groups = getOrgNavGroups(org.slug)
  return filterNavGroups(groups, membership)
}