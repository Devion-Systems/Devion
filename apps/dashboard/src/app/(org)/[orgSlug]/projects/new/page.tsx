...
const PROJECT_TYPES: { type: ProjectType; label: string; description: string; icon: React.ElementType }[] = [
  { type: 'git',    label: 'Git-Repository',  description: 'Verbinde ein GitHub-, GitLab- oder Gitea-Repository',  icon: GitBranch },
  { type: 'docker', label: 'Docker-Image',     description: 'Deploye direkt ein Docker-Image aus einer Registry',     icon: Upload },
  { type: 'blank',  label: 'Leeres Projekt',   description: 'Starte ohne Vorlage und konfiguriere alles manuell',     icon: FolderGit2 },
]
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const res = await fetch(`${baseUrl}/organizations/${orgSlug}/projects`, {
        ...
        body: JSON.stringify({ name, description, type, gitUrl, branch }),
      })
    ...
    <div className="mx-auto max-w-2xl space-y-8 p-6">
      ...
      <div className="space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {PROJECT_TYPES.map(({ type: t, label, description: desc, icon: Icon }) => (
            <button
              key={t}
              type="button"
              onClick={() => setType(t)}
            ...
            >
              <div className={cn(
                ...
                <Icon className={cn('h-4 w-4', type === t ? 'text-[#0984e3]' : 'text-zinc-400')} />
              </div>
              <div>
                <p className={cn('text-sm font-medium', type === t ? 'text-zinc-100' : 'text-zinc-300')}>
                  {label}
                </p>
      ...
      <div className="space-y-4">
        <div>
          <label className="mb-1.5 block text-sm font-medium text-zinc-300">Projektname *</label>
          <input
            type="text"
            value={name}
          ...
          />
          <p className="mt-1 text-[11px] text-zinc-600">Nur Kleinbuchstaben, Zahlen und Bindestriche</p>
        </div>

        <div>
          <label className="mb-1.5 block text-sm font-medium text-zinc-300">Beschreibung</label>
          <input
            type="text"
            value={description}
        ...
        {type === 'git' && (
          <>
            <div>
              <label className="mb-1.5 block text-sm font-medium text-zinc-300">Repository-URL</label>
