...
/**
 * A project-owned persistent storage resource. Runtime names are generated by
 * Devion and deliberately never supplied by an application user.
 */
export const volumes = pgTable(
  "volumes",
  {
    ...
    backend: text("backend", { enum: ["docker_local"] }).notNull().default("docker_local"),
    status: text("status", { enum: ["available", "in_use", "unavailable", "error", "deleting", "legacy"] })
      .notNull()
      .default("available"),
    // Kept as a plain identifier here to avoid a projects <-> control-plane
    // schema import cycle. The database migration still enforces the FK.
    nodeId: text("node_id"),
  ...
  (table) => [
    uniqueIndex("volumes_project_name_uidx").on(table.projectId, table.name),
    uniqueIndex("volumes_runtime_name_uidx").on(table.runtimeName),
    index("volumes_project_status_idx").on(table.projectId, table.status),
    index("volumes_node_idx").on(table.nodeId),
  ],
);
...
export const applicationVolumeMounts = pgTable(
  "application_volume_mounts",
  {
    id: text("id").primaryKey(),
    applicationId: text("application_id").notNull().references(() => applications.id, { onDelete: "cascade" }),
    /** Null only for mounts created before the managed-volume migration. */
    volumeId: text("volume_id").references(() => volumes.id, { onDelete: "restrict" }),
    /** Legacy runtime name retained solely to keep pre-V1 deployments usable. */
  ...
  (table) => [
    ...
    uniqueIndex("application_volume_mounts_volume_uidx").on(table.volumeId),
    index("application_volume_mounts_application_idx").on(table.applicationId),
  ],
);

/** Immutable managed-volume references captured with each deployment revision. */
export const deploymentVolumeMounts = pgTable(
  "deployment_volume_mounts",
  {
    id: text("id").primaryKey(),
    // The migration enforces the FK. Keeping this field local avoids a schema
    // cycle because deployments are declared in control-plane.ts.
    deploymentId: text("deployment_id").notNull(),
    volumeId: text("volume_id").notNull().references(() => volumes.id, { onDelete: "restrict" }),
    mountPath: text("mount_path").notNull(),
    readOnly: boolean("read_only").notNull().default(false),
    createdAt: timestamp("created_at").defaultNow().notNull(),
  },
  (table) => [
    uniqueIndex("deployment_volume_mounts_path_uidx").on(table.deploymentId, table.mountPath),
    index("deployment_volume_mounts_volume_idx").on(table.volumeId),
  ],
);
